...
## 2026-08-02 - Evidence-led production security and performance hardening

**Context:** A fresh audit of `hilivingmgl.mn` measured a mobile Lighthouse performance score of 67, LCP of 4.7 seconds, and CLS of 0.185. The main Vite JavaScript was transferred uncompressed, the API-loaded hero inserted without reserved space, stable root public images had no cache lifetime, and `robots.txt`/favicon requests fell through to the SPA. Runtime dependency scanning also matched the HTML sanitizer used for public rich content and Flyway's transitive Jackson Databind against published advisories.
...
## 2026-09-17 - Paint a static shell and remove the initial-path animation runtime

**Context:** A live landing-page trace showed that the HTML became interactive well before first contentful paint. The source document had an empty React root, so the browser had no content to paint while the stylesheet and JavaScript loaded. The initial graph also included `motion` for only a hero fade/scale and section reveal, even though the page already reserved hero geometry, prioritized the visible hero image, deferred below-the-fold sections, and lazy-loaded secondary images.

**Decision:** Render a small, non-interactive header and reserved hero frame directly in `index.html` with only their critical styles inline; let React replace it during normal `createRoot` startup. Retain lazy route and below-the-fold section boundaries. Replace `motion` with native CSS transitions/keyframes and `IntersectionObserver`, preserving reduced-motion behavior and the once-per-tab hero entrance state.

**Consequences:** A cold navigation can paint recognizable storefront geometry before JavaScript execution, and the measured local initial JavaScript falls from 121.9 KiB to 79.1 KiB gzip. The bootstrap shell deliberately has no controls or API data, so it cannot diverge in navigation behavior or become a second application. External CSS and server-managed banner/content requests remain asynchronous; production FCP/LCP must be re-measured after deployment, and extension-injected DOM must be excluded from the baseline.
