...
Frontend validation uses `npm ci`, ESLint, Vitest/Testing Library HTTP-boundary tests, TypeScript compilation, and a Vite production build. The current 81 tests preserve catalog, account, administration, and commerce coverage and include persistent catalog-shell navigation, catalog-aware scrolling, mobile icon-triggered category switching, Mongolian admin interactions, address-map cleanup and dependent administrative selections, QPay requests, QR/deeplinks, checkout labels, payment checks, and safe idempotency-key retry behavior. Isolated browser rehearsals cover the 390×844 category menu, category-to-brand navigation without hero refetches, customer registration/address/cart/checkout, pending and paid transitions, customer history, callback replay, and ADMIN list/detail visibility.
