...
Backend validation uses Maven with compiler release 21, PostgreSQL Testcontainers, Flyway through V12, Hibernate validation, repository/service/controller/security coverage, and JAR packaging. Coverage includes immutable media cache headers, the QPay V2 JSON contract, durable QR/deeplink creation, ownership-scoped order listing, public callback verification through `payment/check`, exact paid-state confirmation, confirmation-email timing, and exact-once stock restoration after initiation failure. On 2026-07-31 all 65 tests and JAR packaging passed inside exact Temurin Java 21.0.11, matching the Hostinger runtime major version.
