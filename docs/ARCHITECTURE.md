...
Spring serves `GET` and `HEAD /media/**` from the configured root during development with `Cache-Control: max-age=31536000, public, immutable`. UUID filenames are never overwritten, so this one-year policy cannot make a replaced image URL stale. Directory listing and non-read methods are unavailable. Production NGINX must preserve the same immutable cache contract while serving the URL space directly with a read-only `alias /var/lib/hiliving/uploads/`, denying dotfiles, disabling auto-indexing, and keeping `/api` routing ahead of the SPA fallback. Spring must not expose an unrelated working directory.
...
The current workstation contains explicitly requested local test identities plus a production-looking storefront demo dataset. These records exist only in the local PostgreSQL volume and are not shipped through Flyway, application startup, tests, or production deployment. The storefront dataset owns exactly nine active products across the existing nine categories, one primary image per product, two active banners, and four published news articles. Its JPEGs live in the configured external media root and are served through same-origin immutable `/media/...` URLs; source and license traceability is maintained in `docs/DEMO_CONTENT_SOURCES.md`. Product copy, pricing, stock, brands, banners, and news remain fictional presentation data. Customer accounts, orders, and immutable order snapshots are outside this demo-content replacement boundary.
