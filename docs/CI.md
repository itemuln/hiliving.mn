...
Frontend tests mock the HTTP boundary and cover existing catalog/account states plus admin guards, disabled future navigation, the responsive shell, multipart/CSRF behavior, upload progress/failure/retry, four product slots, media-backed forms, the absence of banner scheduling controls, and mobile banner sources. Backend tests use PostgreSQL 17 Testcontainers, apply Flyway V1–V5, start Hibernate with schema validation, and exercise catalog, identity, administration authorization, media validation/processing/storage/serving/cleanup, public visibility, banners, and news.
