...
import { jsonResponse, productDetailDto, productPageEnvelope, productSummaryDto } from '../test/catalogFixtures'
...
describe('catalog API adapter', () => {
  it('serializes every supported product filter with controlled names', () => {
    expect(serializeProductQuery({
      ...
      featured: true,
      sort: 'price_asc',
    })).toBe('page=1&size=16&category=cleaning&brand=tez-blanc&search=surface+cleaner&featured=true&sort=price_asc')
  })

  it('loads and maps a paginated product response into the frontend model', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(productPageEnvelope([productSummaryDto])))
  ...
  it('maps categories to presentation-safe icon metadata', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({
      data: [{ id: 2, name: 'Cleaning', slug: 'cleaning', parentSlug: 'household', displayOrder: 20 }],
    })))
  ...
  it('normalizes backend 404 and server responses without exposing response messages', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(jsonResponse({ error: { code: 'RESOURCE_NOT_FOUND', message: 'database row missing' } }, 404))
      .mockResolvedValueOnce(jsonResponse({ error: { code: 'INTERNAL_ERROR', message: 'SQL connection details' } }, 500))
    vi.stubGlobal('fetch', fetchMock)
  ...
  it('rejects malformed successful envelopes as safe adapter errors', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ unexpected: productDetailDto })))
