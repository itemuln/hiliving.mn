...
function mapProduct(product: ProductSummaryDto): CatalogProduct {
  return {
    ...
    imageScale: product.primaryImageScale,
...
function mapProductDetail(product: ProductDetailDto): CatalogProductDetail {
  ...
  return {
    ...
    listPrice: product.price,
    currentPrice: product.effectiveCustomerPrice,
    imageUrl: primaryImage?.imageUrl ?? '/product-cleaner.svg',
    imageScale: primaryImage?.displayScale ?? 100,
    category: product.category,
    brand: product.brand,
    featured: product.featured,
    sku: product.sku,
    membershipDiscountPercentage: product.membershipDiscountPercentage,
    ...
    images,
    relatedProducts: product.relatedProducts.map(mapProduct),
    createdAt: product.createdAt,
    updatedAt: product.updatedAt,
  };
}
...
export async function fetchProductBySlug(slug: string, signal?: AbortSignal) {
  const product = await request<ProductDetailDto>(
