...
export function VerifyEmailPage() {
  ...
  return (
    <AuthPageShell
      ...
      subtitle="HiLiving бүртгэлийн имэйл хаягийн баталгаажуулалт."
    >
      {state === 'checking' ? (
      ...
      {message ? (
        <p
          ...
          className={`rounded-xl p-5 text-sm leading-6 ${
            state === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-800'
          }`}
        >
          {message}
        </p>
      ...
      {state === 'idle' && auth.status === 'authenticated' && !auth.user.emailVerified ? (
        <button className={primaryButtonClass} onClick={() => void resend()}>
          Баталгаажуулах имэйл дахин авах
        </button>
      ) : null}
      {state === 'error' && auth.status === 'authenticated' && !auth.user.emailVerified ? (
        <button className={`${primaryButtonClass} mt-5`} onClick={() => void resend()}>
          Шинэ холбоос авах
        </button>
      ) : null}
      {state === 'idle' && auth.status !== 'authenticated' ? (
        <p className="text-sm leading-6 text-neutral-600">
          Шинэ холбоос авахын тулд{' '}
          <Link className="font-medium text-brand-600 underline" to="/login">
            нэвтэрнэ үү
          </Link>
          .
        </p>
      ...
      <p className="mt-5 text-center text-sm">
        <Link className="font-medium text-brand-600 hover:underline" to="/account">
          Миний бүртгэл
        </Link>
      </p>
    </AuthPageShell>
