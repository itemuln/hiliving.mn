...
export function CategoryProductsPage() {
  ...
  const hasUnknownSlug = categoriesResource.status === 'success' && Boolean(categorySlug && !activeCategory)
  ...
  const navigation = categoriesResource.status === 'loading'
    ? <CatalogNavigationSkeleton />
    : <Sidebar items={sidebarCategories} activeSlug={activeSlug} basePath="/categories" variant="category" />
  const mobileNavigation = categoriesResource.status === 'loading'
        ...
        <CatalogLayout sidebar={navigation} mobileNavigation={mobileNavigation}>
          ...
          {categoriesResource.status === 'error' || productsResource.status === 'error' ? (
            ...
            <>
              ...
              <CatalogPagination currentPage={productsResource.data.page + 1} totalPages={productsResource.data.totalPages} />
            </>
          ) : (
            <CatalogEmptyState title="Бүтээгдэхүүн олдсонгүй" description="Хайлтын нөхцөлөө өөрчлөөд дахин оролдоно уу." />
          )}
        </CatalogLayout>
      </main>
      <Footer />
