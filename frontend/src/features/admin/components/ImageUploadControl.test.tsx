...
describe('ImageUploadControl', () => {
  ...
  it('uploads selected files, reports progress, previews safely, and persists only the returned URL', async () => {
    ...
    const pending = vi.fn();
    let complete!: () => void;
    vi.mocked(api.uploadMediaImage).mockImplementation(
      (_file, _purpose, options) =>
        new Promise((resolve) => {
          options?.onProgress?.(55);
          complete = () =>
            resolve({
              id: 7,
              storageKey: 'brands/generated.png',
              url: '/media/brands/generated.png',
    ...
    render(
      <ImageUploadControl
        label="Brand logo"
        purpose="BRAND"
        value="https://legacy.example/logo.png"
        onChange={change}
