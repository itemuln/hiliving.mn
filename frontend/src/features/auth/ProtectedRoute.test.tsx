...
describe('protected account routes', () => {
  ...
  it('rejects customers from admin routes and allows admins', () => {
    const customer: AuthContextValue = { ...anonymous, state: { status: 'authenticated', user: authenticatedUser } }
    const { unmount } = render(<AuthContext.Provider value={customer}><MemoryRouter initialEntries={['/admin']}><Routes><Route path="/admin" element={<ProtectedRoute admin><span>admin area</span></ProtectedRoute>} /><Route path="/account" element={<span>account home</span>} /></Routes></MemoryRouter></AuthContext.Provider>)
    expect(screen.getByText('account home')).toBeInTheDocument()
    unmount()
    const admin: AuthContextValue = { ...customer, state: { status: 'authenticated', user: { ...authenticatedUser, role: 'ADMIN' } } }
    render(<AuthContext.Provider value={admin}><MemoryRouter initialEntries={['/admin']}><ProtectedRoute admin><span>admin area</span></ProtectedRoute></MemoryRouter></AuthContext.Provider>)
