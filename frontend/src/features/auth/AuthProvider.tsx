...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const expired = () => { setHydrationError(false); setState({ status: 'anonymous', user: null }) }
  ...
  const value = useMemo(() => ({
    ...
    async logout() {
      try { await accountApi.logout() } finally { setHydrationError(false); setState({ status: 'anonymous', user: null }) }
    },
    refresh,
    replaceUser(user: AuthenticatedUser) { setState({ status: 'authenticated', user }) },
