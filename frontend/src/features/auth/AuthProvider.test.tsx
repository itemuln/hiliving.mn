...
function Probe() {
  ...
  return <div><span>{state.status}</span><span>{hydrationError ? 'hydration-error' : 'hydration-ok'}</span>{state.user ? <span>{state.user.email}</span> : null}<button onClick={() => void login({ identifier: 'x', password: 'y' })}>login</button><button onClick={() => void logout()}>logout</button></div>
}
