...
const context: AuthContextValue = { state: { status: 'authenticated', user: authenticatedUser }, hydrationError: false, login: vi.fn(), register: vi.fn(), logout: vi.fn(), refresh: vi.fn(), replaceUser }

describe('account forms', () => {
  it('updates profile data and refreshes auth state', async () => {
    document.cookie = 'XSRF-TOKEN=token; path=/'
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(accountJson({ data: { ...authenticatedUser, firstName: 'Updated' } })))
    render(<MemoryRouter><AuthContext.Provider value={context}><ProfileForm /></AuthContext.Provider></MemoryRouter>)
  ...
  it('confirms a successful password change', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Одоогийн нууц үг'), { target: { value: 'Current1234' } })
  ...
  it('renders empty addresses then creates a default address', async () => {
    document.cookie = 'XSRF-TOKEN=token; path=/'
    const created = { id: 1, label: 'Гэр', cityOrProvince: 'Улаанбаатар', districtOrSoum: 'СБД', khorooOrBag: null, addressLine: 'Энхтайвны өргөн чөлөө', additionalDetails: null, recipientName: 'Temuulen', recipientPhone: '+97699112233', defaultAddress: true, createdAt: '', updatedAt: '' }
    ...
    fireEvent.change(screen.getByLabelText('Дэлгэрэнгүй хаяг'), { target: { value: 'Энхтайвны өргөн чөлөө' } })
    fireEvent.change(screen.getByLabelText('Хүлээн авагч'), { target: { value: 'Temuulen' } })
    fireEvent.change(screen.getByLabelText('Хүлээн авагчийн утас'), { target: { value: '99112233' } })
