...
export function AddressForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    await onSubmit({
      ...
      cityOrProvince: String(data.get('cityOrProvince') ?? ''),
      districtOrSoum: String(data.get('districtOrSoum') ?? ''),
      khorooOrBag: String(data.get('khorooOrBag') ?? '') || null,
      addressLine: String(data.get('addressLine') ?? ''),
      additionalDetails: String(data.get('additionalDetails') ?? '') || null,
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">
          ...
          <input
            className={fieldClass}
            name="cityOrProvince"
            defaultValue={editing?.cityOrProvince}
            required
          />
        </label>
        <label className="text-sm font-medium">
          ...
          <input
            className={fieldClass}
            name="districtOrSoum"
            defaultValue={editing?.districtOrSoum}
            required
          />
        </label>
        <label className="text-sm font-medium">
          ...
          <input
            className={fieldClass}
            name="khorooOrBag"
            defaultValue={editing?.khorooOrBag ?? ''}
          />
        </label>
        ...
        <label className="text-sm font-medium sm:col-span-2">
          ...
          <textarea
            ...
            defaultValue={editing?.additionalDetails ?? ''}
          />
        </label>
        <label className="text-sm font-medium">
          Хүлээн авагч
      ...
      <label className="mt-4 flex items-center gap-2 text-sm">
        <input type="checkbox" name="defaultAddress" defaultChecked={editing?.defaultAddress} />{' '}
        Үндсэн хаяг болгох
      </label>
