...
export function NewsCard({ item }: Readonly<NewsCardProps & { item: NewsCardProps['item'] & { slug?: string } }>) {
  return (
    <article className="group min-w-0">
      <a href={`/news/${item.slug ?? item.id}`} className="block overflow-hidden rounded-xl bg-neutral-100">
      ...
      <h3 className="mt-4 text-sm font-medium leading-snug text-neutral-700 md:text-base">
        <a href={`/news/${item.slug ?? item.id}`} className="transition-colors duration-300 ease-out hover:text-brand-500">{item.title}</a>
      </h3>
      <p className="mt-1.5 text-xs leading-relaxed text-neutral-500 md:text-sm">{item.description}</p>
      <a href={`/news/${item.slug ?? item.id}`} className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-brand-500 transition-all duration-300 ease-out hover:gap-2 hover:text-brand-600 md:text-sm">
