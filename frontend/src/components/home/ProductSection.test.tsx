...
function renderSection() {
  return render(<MemoryRouter><ProductSection /></MemoryRouter>)
}
...
describe('ProductSection', () => {
  ...
  it('renders products returned by the catalog API', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse(productPageEnvelope([productSummaryDto]))))

    renderSection()

    expect(await screen.findAllByText('Plant-Based Household Cleaner')).toHaveLength(2)
    expect(screen.getAllByRole('link', { name: /Plant-Based Household Cleaner/ })[0]).toHaveAttribute(
      'href',
      '/products/plant-based-household-cleaner',
    )
  })
  ...
  it('renders an explicit empty state for an empty successful page', async () => {
    ...
    expect(await screen.findByText('Онцлох бүтээгдэхүүн одоогоор байхгүй байна')).toBeInTheDocument()
  })
