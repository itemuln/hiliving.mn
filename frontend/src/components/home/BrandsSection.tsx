      ...
      <Container>
        ...
        <div className="mt-8 md:mt-10">
          {resource.status === 'loading' ? (
            <div aria-label="Брэнд ачаалж байна" role="status" className="grid grid-cols-4 gap-2 sm:gap-3 md:grid-cols-5 md:gap-5">
              {Array.from({ length: 10 }, (_, index) => (
                <div key={index} aria-hidden="true" className="aspect-[2.15/1] animate-pulse rounded-md bg-neutral-100" />
              ))}
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-3 md:grid-cols-5 md:gap-5">
              {resource.data.map((brand, index) => (
                ...
                >
                  {brand.logoUrl ? (
                    <img src={brand.logoUrl} alt={brand.name} loading="lazy" className="max-h-[70%] max-w-[80%] object-contain" />
                  ) : (
            ...
            <CatalogEmptyState compact title="Брэнд одоогоор байхгүй байна" description="Каталогийн брэнд нэмэгдэх үед энд харагдана." />
          )}
        </div>
      </Container>
    </SectionReveal>
