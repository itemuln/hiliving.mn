...
public class AdminBrandService {
    ...
    @Transactional public AdminBrandResponse create(AdminBrandRequest r) {
        ensureSlug(r.slug(),null); BrandEntity b=BrandEntity.create(r.name().trim(),r.slug(),clean(r.logoUrl()),clean(r.bannerImageUrl()),r.active());
        b.update(r.name().trim(),r.slug(),clean(r.logoUrl()),clean(r.bannerImageUrl()),clean(r.description()),r.active()); brands.saveAndFlush(b);
    ...
    @Transactional public AdminBrandResponse update(Long id, AdminBrandRequest r) {
        BrandEntity b=require(id); ensureSlug(r.slug(),id); boolean wasActive=b.isActive(); boolean logoChanged=!Objects.equals(b.getLogoUrl(),clean(r.logoUrl())); boolean bannerChanged=!Objects.equals(b.getBannerImageUrl(),clean(r.bannerImageUrl()));
        b.update(r.name().trim(),r.slug(),clean(r.logoUrl()),clean(r.bannerImageUrl()),clean(r.description()),r.active()); brands.flush();
        audit.record(wasActive&&!r.active()?"BRAND_DEACTIVATED":"BRAND_UPDATED","BRAND",id,r.slug()); if(logoChanged)audit.record("BRAND_LOGO_CHANGED","BRAND",id,null); if(bannerChanged)audit.record("BRAND_BANNER_CHANGED","BRAND",id,null); return response(b);
    }
    ...
    private AdminBrandResponse response(BrandEntity b){return new AdminBrandResponse(b.getId(),b.getName(),b.getSlug(),b.getLogoUrl(),b.getBannerImageUrl(),b.getDescription(),b.getDisplayOrder(),b.isActive(),products.countByBrandId(b.getId()));}
