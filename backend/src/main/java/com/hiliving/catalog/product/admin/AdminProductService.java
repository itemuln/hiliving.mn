...
public class AdminProductService {
    ...
    private final ProductRepository products; private final CategoryRepository categories; private final BrandRepository brands; private final AuditService audit; private final ProductIdentifierGenerator identifiers; private final HtmlContentSanitizer htmlSanitizer;
    public AdminProductService(ProductRepository products,CategoryRepository categories,BrandRepository brands,AuditService audit,ProductIdentifierGenerator identifiers,HtmlContentSanitizer htmlSanitizer){this.products=products;this.categories=categories;this.brands=brands;this.audit=audit;this.identifiers=identifiers;this.htmlSanitizer=htmlSanitizer;}
    ...
    @Transactional public AdminProductResponse create(AdminProductRequest r){validate(r);CategoryEntity c=category(r.categoryId());BrandEntity b=brand(r.brandId());String name=clean(r.name());String description=cleanNullable(htmlSanitizer.sanitize(r.description()));
    ...
    @Transactional public AdminProductResponse update(Long id,AdminProductRequest r){ProductEntity p=require(id);validate(r);CategoryEntity c=category(r.categoryId());BrandEntity b=brand(r.brandId());
        boolean priceChanged=p.getPrice().compareTo(r.basePrice())!=0||!Objects.equals(p.getDiscountPrice(),r.discountPrice());boolean inventoryChanged=p.getStockQuantity()!=r.stockQuantity()||p.getLowStockThreshold()!=r.lowStockThreshold();boolean eligibilityChanged=p.isMembershipDiscountEligible()!=r.membershipDiscountEligible();boolean imagesChanged=!p.getImages().stream().map(i->i.getImageUrl()+"\u0000"+i.isPrimaryImage()+"\u0000"+i.getDisplayScale()).toList().equals(r.images().stream().sorted(Comparator.comparingInt(AdminProductImageRequest::sortOrder)).map(i->i.imageUrl().trim()+"\u0000"+i.primaryImage()+"\u0000"+i.displayScale()).toList());
        String description=cleanNullable(htmlSanitizer.sanitize(r.description()));p.update(clean(r.name()),descriptionSummary(description),description,r.basePrice(),r.discountPrice(),c,b,r.lifecycle(),r.featured(),r.newProduct(),r.active(),r.stockQuantity(),r.lowStockThreshold(),r.membershipDiscountEligible());p.clearImages();products.flush();addImages(p,r.images());products.flush();
    ...
    private void addImages(ProductEntity p,List<AdminProductImageRequest> images){images.stream().sorted(Comparator.comparingInt(AdminProductImageRequest::sortOrder)).forEach(i->p.addImage(i.imageUrl().trim(),cleanNullable(i.altText()),i.sortOrder(),i.primaryImage(),i.displayScale()));}
    ...
    private AdminProductResponse response(ProductEntity p){return new AdminProductResponse(p.getId(),p.getName(),p.getSlug(),p.getProductCode(),p.getShortDescription(),p.getDescription(),p.getPrice(),p.getDiscountPrice(),ref(p.getCategory()),ref(p.getBrand()),p.getStatus(),p.getStockQuantity(),p.getLowStockThreshold(),InventoryState.of(p.getStockQuantity(),p.getLowStockThreshold()),p.isFeatured(),p.isNewProduct(),p.isActive(),p.isMembershipDiscountEligible(),p.getImages().stream().map(i->new ProductImageResponse(i.getId(),i.getImageUrl(),i.getAltText(),i.getDisplayOrder(),i.isPrimaryImage(),i.getDisplayScale())).toList(),p.getCreatedAt(),p.getUpdatedAt());}
    ...
    private static ApiRequestException validation(String field,String message){return new ApiRequestException(HttpStatus.BAD_REQUEST,"VALIDATION_ERROR",field+": "+message);}private static String clean(String v){return v.trim();}private static String cleanNullable(String v){return v==null||v.isBlank()?null:v.trim();}private static String descriptionSummary(String value){if(value==null)return null;String text=WHITESPACE_PATTERN.matcher(HTML_TAG_PATTERN.matcher(value).replaceAll(" ").replace("&nbsp;"," ").replace("&#160;"," ").replace("&amp;","&").replace("&lt;","<").replace("&gt;",">").replace("&quot;","\"").replace("&#39;","'")).replaceAll(" ").trim();if(text.isBlank())return null;int codePoints=text.codePointCount(0,text.length());int end=text.offsetByCodePoints(0,Math.min(500,codePoints));return text.substring(0,end);}
}
