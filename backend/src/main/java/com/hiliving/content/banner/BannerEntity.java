...
public class BannerEntity {
    ...
    @Enumerated(EnumType.STRING) @Column(nullable=false,length=20) private BannerPlacement placement;
    ...
    public void update(BannerRequest r){title=r.title().trim();subtitle=clean(r.subtitle());imageUrl=r.imageUrl().trim();mobileImageUrl=clean(r.mobileImageUrl());linkUrl=clean(r.linkUrl());linkLabel=clean(r.linkLabel());placement=r.placement();displayOrder=r.sortOrder();active=r.active();startsAt=r.startsAt();endsAt=r.endsAt();}
    ...
    public Long getId(){return id;}public String getTitle(){return title;}public String getSubtitle(){return subtitle;}public String getImageUrl(){return imageUrl;}public String getMobileImageUrl(){return mobileImageUrl;}public String getLinkUrl(){return linkUrl;}public String getLinkLabel(){return linkLabel;}public BannerPlacement getPlacement(){return placement;}public int getDisplayOrder(){return displayOrder;}public boolean isActive(){return active;}public Instant getStartsAt(){return startsAt;}public Instant getEndsAt(){return endsAt;}public Instant getCreatedAt(){return createdAt;}public Instant getUpdatedAt(){return updatedAt;}
}
